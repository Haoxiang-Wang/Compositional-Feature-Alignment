...
def tarfile_to_samples_nothrow(clip_finetune, handler=log_and_continue):
    # NOTE this is a re-impl of the webdataset impl with group_by_keys that doesn't throw
    streams = url_opener(clip_finetune, handler=handler)
...
class detshuffle2(wds.PipelineStage):
    ...
    def run(self, clip_finetune):
        ...
        rng.seed(seed)
        return _shuffle(clip_finetune, self.bufsize, self.initial, rng)
        ...
        if is_train:
            ...
            if not num_samples:
                raise RuntimeError(
                    ...
                    'Please specify via `--train_num_samples` if no dataset length info present.')
        else:
